...
CallCenterSchema.pre('save', function(next){
    var now = new Date();
    this.updated_at = now;
    if ( !this.created_at ) {
        this.created_at = now;
 ...
 * @param callBack
 */
CallCenterSchema.statics.addNew = function (eventData, callBack) {
 ...
 * @param callBack
 */
CallCenterSchema.statics.getRecordById = function(id, callBack){
 ...
 * @param callBack
 */
CallCenterSchema.statics.get = function (filter, fields, callBack) {

    var callCenter = new this();
    var options = { multi: true };

    callCenter.find(filter, fields, options, function (err, events) {
        if (err){
            console.log(err);
            callBack({status:400,error:err});
        } else {
 ...
 * @param callBack
 */
CallCenterSchema.statics.updateCallRecord = function (filter, value, callBack) {
