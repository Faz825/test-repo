    ...
    describe('User', function () {
        ...
        it('doSignup', function (done) {
            agent.post('/doSignup')
                .send({
                    ...
                    password: '123456',
                    status: 7,
                    secretary: '56b8a00ebee8f66920003d46'
                })
                .end(function (err, res) {
                    user = res.body.user;
        ...
        it('saveSecretary', function (done) {
            agent.post('/secretary/save')
                .send({secretary: '56b8a00ebee8f66920003d46'})
                .end(function (err, res) {
                    expect(res.body.status).to.equal('success');
                    expect(res.body).to.have.property('user');
        ...
        it('saveGeneralInfo', function (done) {
            agent.post('/general-info/save')
                .send({dob: "2016-05-03T23:29:58.527Z", country: 'USA', zip: '10100'})
                .end(function (err, res) {
                    expect(res.body.status.code).to.equal(200);
                    expect(res.body).to.have.property('user');
        ...
        it('deleteEducationDetail', function (done) {
            agent.get('/education-info/delete')
                .set({_education_id: "56c321a42ab09c7b09034e85", _userId: "56c2d6038c920a41750ac4db"})
                .end(function (err, res) {
                    expect(res.body.status).to.equal(200);
                    done();
                });
        ...
        it('validateToken', function (done) {
            agent.get('/forgot-password/reset?token=')
                .end(function (err, res) {
                    expect(res.body.status.code).to.equal(200);
                    done();
                });
        ...
        it('resetPassword', function (done) {
            agent.post('/change-password?token=')
                .end(function (err, res) {
                    expect(res.body.status.code).to.equal(200);
                    done();
                });
